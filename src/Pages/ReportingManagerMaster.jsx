    ...
    managerForm
      ...
      .then((values) => {
        try {
          const requestData = {
            ...values,
            id: editingManager ? editingManager.reporting_manager_id : null,
          };
          const url = editingManager
            ? `${editManager}/${editingManager.reporting_manager_id}`
            : `${createManager}`;
          const response = axios.post(url, requestData);
          if (response.status) {
            if (
              editingManager &&
              editingManager.reporting_manager_id !== null
            ) {
                  ...
                  >
                    ...
                    <Row gutter={[8, 4]}>
                      ...
                      <Col span={12}>
                        <Form.Item
                          name="department"
                          label={<span className="text-info">Department</span>}
                          rules={[
                            {
                              required: true,
                            ...
                            },
                            {
                              pattern: /^[&,.\-_\w\s]{1,50}$/,
                              message:
                                "Please enter a valid Department Name (up to 50 characters, only &, , ., -, _ special characters are allowed)",
                            },
                          ]}
                        >
                          <Input />
                        ...
                        <tr key={data.reporting_manager_id}>
                          ...
                          <td className="d-flex gap-2">
                            ...
                            <button
                              ...
                              onClick={() =>
                                deleteManagerHandler(data.reporting_manager_id)
                              }
                            >
                              Delete
                            </button>
