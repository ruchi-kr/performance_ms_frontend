...
import { getAllModules, createModule, editModule, deleteModule, getAllProjects } from '../Config.js';
...
const ModuleMaster = () => {
    ...
    const getAllModulesHandler = async (page) => {
        try {
            const response = await axios.get(`${getAllModules}?page=${page}&pageSize=${pageSize}&name=${projectName}`);
    ...
    const moduleFormSubmit = (values) => {
        moduleForm.validateFields()
            .then((values) => {
                try {
                    const requestData = { ...values, id: editingModule ? editingModule.module_id : null };
                    const url = editingModule ? `${editModule}/${editingModule.module_id}` : `${createModule}`;
                        ...
                        <div className="row my-5">
                            <div className="col-11 mx-auto">
                                ...
                                <div className='d-flex justify-content-between'>
                                    ...
                                    <button className='btn btn-sm btn-info d-flex align-items-center' onClick={openModuleAdd} >
                                ...
                                {/* modal */}
                                <Modal title={editingModule ? 'Edit Project Module' : 'Add Project Module'} visible={modalVisible}
                                    ...
                                    centered
                                >
                                    <Form form={moduleForm} onFinish={moduleFormSubmit} layout="vertical" disabled={formDisabled}>
                                        <Row gutter={[8, 4]}>

                                            <Col span={12}>
                                                <Form.Item name="project_name" label={<span className='text-info mt-3'>Project Name</span>}
                                                    rules={[
                                                ...
                                                return (
                                                    <tr key={data.designation_id}>
                                                        ...
                                                        <td className=''>
                                                            ...
                                                            <EyeOutlined onClick={() => openModuleView(data)} style={{ color: "blue", marginRight: "1rem" }} />
                                                            <EditOutlined onClick={() => openModuleEdit(data)} style={{ color: "blue", marginRight: "1rem" }} />
                                                            <DeleteOutlined onClick={() => deleteModuleHandler(data.designation_id)} style={{ color: "red", marginRight: "1rem" }} />
                                                        </td>
