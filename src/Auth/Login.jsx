    ...
    try {
      ...
      if (result.status === 200) {
        ...
        sessionStorage.setItem(
          "id",
          JSON.stringify(result.data.result.user.id)
        );
        sessionStorage.setItem(
          "email_id",
          JSON.stringify(result.data.result.user.email_id)
        );
        sessionStorage.setItem(
          "user_type",
          JSON.stringify(result.data.result.user.user_type)
        );
        sessionStorage.setItem(
          "role",
          JSON.stringify(result.data.result.user.role)
        );
        sessionStorage.setItem(
          "status",
    ...
    <div className="container-fluid loginbg" style={{ height: "100vh" }}>
      <div className="row h-100">
        <div className="col-lg-6 form-container col-xl-4 col-12 col-md-8 p-5 rounded-4 blurloginbox mx-auto my-auto shadow-sm">
          ...
          >
            <span className="fw-bolder text-dark">Welcome</span>
            <br />
            {/* <span className='textcolor fw-bolder'>Earthood</span> */}
          </p>
          <h5 className="text-info text-center fw-bolder">
          ...
          >
            {({ handleSubmit, isSubmitting }) => (
              <form onSubmit={handleSubmit}>
                <div className="mb-3">
                  <label htmlFor="email_id" className="form-label text-info">
                    Email
                  </label>
                  <Field
                    type="text"
                ...
                <div className="d-grid">
                  <button
                    className="btn btn-success border-0 bg-dark text-white"
                    type="submit"
